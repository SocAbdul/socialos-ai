...
def hcl_map(values: dict[str, str]) -> str:
    ...
    for key in sorted(values):
        lines.append(f"  {key.ljust(width)} = {hcl_string(values[key])}")
    lines.append("}")
    return "\n".join(lines)


def hcl_assignments(values: list[tuple[str, str]]) -> str:
    width = max(len(key) for key, _ in values)
    return "\n".join(f"{key.ljust(width)} = {value}" for key, value in values)
...
def main() -> None:
    ...
    foundation_values = [
        ("aws_region", hcl_string(os.environ["AWS_REGION"])),
        ("media_bucket_name", hcl_string(os.environ["S3_MEDIA_BUCKET"])),
        ("media_cors_allowed_origins", f"[{hcl_string(web_origin)}]"),
    ...
    runtime_values = [
        ...
        (
            "staging_api_image",
            hcl_string(os.environ.get("STAGING_API_IMAGE") or "REPLACE_AFTER_STAGING_IMAGE_PUBLICATION"),
        ),
        (
            "staging_web_image",
            hcl_string(os.environ.get("STAGING_WEB_IMAGE") or "REPLACE_AFTER_STAGING_IMAGE_PUBLICATION"),
        ),
        ("staging_desired_count", os.environ.get("STAGING_DESIRED_COUNT") or "1"),
    ]

    contents = f"""{hcl_assignments(foundation_values)}

{hcl_assignments(runtime_values)}
